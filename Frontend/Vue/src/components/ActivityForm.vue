  ...
  <div class="container custom-container mt-4">
    <Form @submit="handleSubmit" v-slot="{ errors }">
      ...
      <div class="form-group">
        ...
        :value="user_ActivityName" @input="updateUserActivityName($event.target.value)" rules="required" />
        ...
        <div class="form-group">
          ...
          <template v-else-if="detail.name === 'Start Time'">
            ...
            <Field type="time" class="input form-control" :id="detail.name + index" :name="detail.name"
              v-model="detail.value" step="1" rules="required" @change="handleTimeChange" />
            <ErrorMessage :name="detail.name" class="error-msg" />
          </template>
      ...
      <button v-if="mode === 'Edit'" type="button" @click="$emit('delete')" class="delete-button btn btn-danger">Delete Activity</button>
      </div>
    </Form>
  </div>
</template>
